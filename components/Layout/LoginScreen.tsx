...
export function LoginScreen() {
  ...
  const [loginTab, setLoginTab]   = useState<'email' | 'password'>('password');
  ...
  const hasPassword = !!MASTER_PASSWORD;
  ...
  const handlePasswordSubmit = (e: React.FormEvent) => {
    ...
    if (!user) {
      setBypassError(`"${nameKey}" not found — use your first name.`);
      return;
    }
  ...
  const btn = (active: boolean): React.CSSProperties => ({
    width: '100%',
    backgroundColor: active ? '#F5C800' : '#1A1A1A',
    border: 'none',
    borderRadius: 10,
    padding: '12px',
      ...
      <div style={{
          ...
          <div>
            <div style={{ textAlign: 'center', marginBottom: 20 }}>
              ...
              <p style={{ color: '#666', fontSize: 12.5, margin: 0, lineHeight: 1.6 }}>
                ...
                Click the link in the email — it will sign you in automatically.
                If you also received a 6-digit code, enter it below.
              </p>
            </div>
              ...
              {code.map((digit, idx) => (
                <input
                  ...
                  style={{
                    ...
                    borderRadius: 10, color: '#F5F5F5', fontSize: 22, fontWeight: 800,
                    textAlign: 'center', outline: 'none', fontFamily: 'monospace', transition: 'border-color 0.15s',
                  }}
            ...
            {codeError && <p style={{ color: '#EF4444', fontSize: 12, textAlign: 'center', margin: '0 0 8px', lineHeight: 1.5 }}>{codeError}</p>}

            <div style={{ display: 'flex', justifyContent: 'center', gap: 16, marginTop: 8 }}>
              <button onClick={() => { setSent(false); setCode(['', '', '', '', '', '']); setCodeError(''); }}
        ...
        /* ── Confirm bypass identity ── */
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <button onClick={confirmBypassLogin} style={btn(true)}>Yes, that&apos;s me →</button>
        ...
        /* ── Main login (configured + Supabase live) ── */
        ) : configured ? (
          <>
            <h2 style={{ color: '#F5F5F5', fontSize: 16, fontWeight: 800, margin: '0 0 1.25rem', textAlign: 'center' }}>
              Sign in to continue
            </h2>

            {/* Tab switcher — only shown when password bypass is available */}
            {hasPassword && (
              <div style={{ display: 'flex', gap: 2, backgroundColor: '#1A1A1A', borderRadius: 10, padding: 3, marginBottom: 20 }}>
                {([['password', '🔑 Password'], ['email', '✉️ Email link']] as const).map(([tab, label]) => (
                  <button
                    key={tab}
                    onClick={() => { setLoginTab(tab); setError(''); setBypassError(''); }}
                    style={{
                      flex: 1, padding: '8px 0', borderRadius: 8, border: 'none',
                      backgroundColor: loginTab === tab ? '#F5C800' : 'transparent',
                      color: loginTab === tab ? '#000' : '#555',
            ...
            {hasPassword && loginTab === 'password' ? (
              <form onSubmit={handlePasswordSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <input
                  type="password" value={bypassPass}
                ...
                <button type="submit" disabled={!bypassName.trim() || !bypassPass.trim()}
                  style={btn(!!(bypassName.trim() && bypassPass.trim()))}>
                  Sign in →
                </button>
              </form>
            ...
            /* Email tab */
              ...
              <form onSubmit={handleEmailSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
                ...
                <p style={{ color: '#555', fontSize: 11.5, margin: 0, lineHeight: 1.5 }}>
                  We&apos;ll email you a sign-in link. Check spam if it doesn&apos;t arrive within a minute.
                </p>
        ...
        /* ── Staging bypass (Supabase not configured) ── */
        ) : (
          <>
            <h2 style={{ color: '#F5F5F5', fontSize: 16, fontWeight: 800, margin: '0 0 1.25rem', textAlign: 'center' }}>
              Sign in to continue
            ...
            <form onSubmit={handlePasswordSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
              <p style={{ color: '#555', fontSize: 12, margin: '0 0 4px', lineHeight: 1.5 }}>
                Enter your <strong style={{ color: '#888' }}>first name</strong>{MASTER_PASSWORD ? ' and the team password' : ''}.
              </p>
              <div style={{ display: 'flex', alignItems: 'stretch' }}>
                <input
                  type="text" value={bypassName}
              ...
              <button
                ...
                style={btn(!!(bypassName.trim() && (!MASTER_PASSWORD || bypassPass.trim())))}
              >Sign in →</button>
            </form>
          </>
        )}
