...
export const LOCAL_USERS: Record<string, UserProfile> = {
  ...
  diego:     { email: 'diego@roofignite.com',      displayName: 'Diego',     userKey: 'diego',     role: 'media_buyer' },
...
export async function getUserProfileByEmail(email: string): Promise<UserProfile | null> {
  ...
  if (supabase) {
    try {
      ...
      // Retry with base fields only (profile columns may not exist yet)
  ...
  // Fall back to LOCAL_USERS so logins work without a DB row
